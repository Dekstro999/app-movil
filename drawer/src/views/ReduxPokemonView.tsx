...
import { loadRandomPokemonThunk } from "../state/redux/pokemonSlice";

export default function ReduxPokemonView() {
  const dispatch = useAppDispatch();
  const { pokemon, isLoading, error } = useAppSelector((state) => state.pokemon);
  const [startedAt, setStartedAt] = useState<number | null>(null);
  const [lastLoadMs, setLastLoadMs] = useState<number | null>(null);

  const handleLoadPokemon = () => {
    setStartedAt(Date.now());
    void dispatch(loadRandomPokemonThunk());
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.description}>Estado global con store + slice + thunk</Text>
